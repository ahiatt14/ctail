...
void m3x3__identity(M3x3 *const m) {
  m3x3__create(
    1, 0, 0,
    0, 1, 0,
    0, 0, 1,
...
float m3x3__minor(int r, int c, M3x3 const *const m) {
  M2x2 sub_cache = {0};
  int sub_indices[4] = {0};
  int sub_index = 0;
...
void m3x3__minors(M3x3 const *const src, M3x3 *const dest) {
  M3x3 temp = {0};
  for (int c = 0; c < 3; c++) {
    for (int r = 0; r < 3; r++) {
  ...
  memcpy(&dest->data[0], &temp.data[0], sizeof(M3x3));
}

void m3x3__cofactors(M3x3 const *const src, M3x3 *const dest) {
  for (int i = 0; i < 9; i++)
    dest->data[i] = (i % 2 == 0) ? src->data[i] : -src->data[i];
}

void m3x3__transpose(M3x3 const *const src, M3x3 *const dest) {
  float temp;
...
float m3x3__determinant(M3x3 const *const m) {
  return
...
static void float_x_m3x3(float s, const M3x3 *src, M3x3 *dest) {
  for (int i = 0; i < 9; i++) dest->data[i] = s * src->data[i];
}

void m3x3__inverse(M3x3 const *const src, M3x3 *const dest) {
  M3x3 minors = {0};
  M3x3 cofactors = {0};
  M3x3 transposed = {0};
